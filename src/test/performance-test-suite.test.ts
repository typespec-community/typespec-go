...
} from "./performance/performance-reporter.js";
...
class PerformanceTestSuite {
  ...
  runCategory(
    ...
    const categoryBenchmarks = PERFORMANCE_BENCHMARKS.filter(
...
describe("Performance Test Suite", () => {
  it("should execute complete performance test suite", () => {
    ...
    console.log(
      `Performance suite completed: ${summary.passedTests}/${summary.totalTests} passed`,
    );
  });
  ...
  it("should execute single benchmark", () => {
    expect(() =>
      testSuite.runSingleBenchmark("basic-simple-user"),
    ).not.toThrow();
  });
});

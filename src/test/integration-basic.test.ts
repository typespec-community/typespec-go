...
describe("Integration Tests - End-to-End TypeSpec to Go Generation", () => {
  ...
  it("Integration Test #1: Simple complete user model workflow", () => {
    ...
    if (result._tag === "success") {
      ...
      expect(goCode).toContain('DisplayName *string `json:"displayName,omitempty"`');
      expect(goCode).toContain('Theme *string `json:"theme,omitempty"`');
      expect(goCode).toContain('TwoFactorEnabled *bool `json:"twoFactorEnabled,omitempty"`');
      expect(goCode).toContain('LoginCount *uint32 `json:"loginCount,omitempty"`');
      expect(goCode).toContain('LastLoginAt *string `json:"lastLoginAt,omitempty"`');
      expect(goCode).toContain('IsVerified *bool `json:"isVerified,omitempty"`');
      ...
      console.log("✅ Integration Test #1 PASSED: Complete user model workflow");
  ...
  it("Integration Test #2: Complex model with all supported TypeSpec types", () => {
    ...
    if (result._tag === "success") {
      ...
      expect(goCode).toContain("package api");
      expect(goCode).toContain("// Auto-generated from TypeSpec model: ComplexModel");
      expect(goCode).toContain("type ComplexModel struct {");
      expect(goCode).toContain("}");
      
      // Verify string types
      expect(goCode).toContain('StringField string `json:"stringField"`');
      expect(goCode).toContain('OptionalString *string `json:"optionalString,omitempty"`');
      ...
      expect(goCode).toContain('Float32Field *float32 `json:"float32Field,omitempty"`');
      ...
      expect(goCode).toContain('OptionalBool *bool `json:"optionalBool,omitempty"`');
      
      // Verify array types
      expect(goCode).toContain('StringArray []string `json:"stringArray"`');
      expect(goCode).toContain('OptionalIntArray *[]int32 `json:"optionalIntArray,omitempty"`');
      ...
      console.log(`📊 Generated comprehensive model with ${complexModel.properties.size} fields`);
      console.log(`🔤 Types tested: String, Int8-64, Uint8-64, Float32/64, Boolean, Arrays`);
      
    } else {
      console.error("❌ Integration Test #2 FAILED:", result.message);
      console.error(`🔧 Resolution: ${result.resolution}`);
      throw new Error(`Complex model integration test failed: ${result.message}`);
    }
  });
  ...
  it("Integration Test #3: Comprehensive error handling scenarios", () => {
    ...
    console.log("✅ Integration Test #3 PASSED: Comprehensive error handling verified");
    console.log(`🛡️ Error types tested: ModelValidationError, SystemError, Invalid types`);
    console.log(`📊 Scenarios tested: 5/5 error handling scenarios`);
    console.log(`🔧 Error handling: Professional discriminated union patterns working`);

...
import { MemoryTestRunner, type MemoryMetrics, type MemoryLeakTestResult } from "./memory/memory-test-runner.js";
...
class MemoryValidationSuite {
  ...
  runFullMemoryValidation(): {
    ...
    const report = this.memoryRunner.generateMemoryReport(efficiencyMetrics, leakTestResult);
    console.log(report);
    
    // Analyze and validate results
    const efficiencyAnalysis = this.memoryValidator.analyzeMemoryEfficiency(efficiencyMetrics);
    const leakAnalysis = this.memoryValidator.detectMemoryLeaks(leakTestResult);
    const thresholdValidation = this.memoryValidator.validateMemoryThresholds(efficiencyMetrics);
    const recommendations = this.memoryValidator.generateOptimizationRecommendations(efficiencyMetrics, leakTestResult);
    const summary = this.memoryValidator.generateMemorySummary(efficiencyMetrics, leakTestResult);
    
    console.log("\n🔍 Memory Analysis Results:");
    console.log(`   Efficiency: ${efficiencyAnalysis.memoryEfficiency} (${efficiencyAnalysis.scalingFactor.toFixed(2)}x scaling)`);
    console.log(`   Leak Status: ${leakAnalysis.leakSeverity} (${leakAnalysis.recommendation})`);
    console.log(`   Threshold Validation: ${thresholdValidation.passed ? '✅ PASSED' : '❌ FAILED'}`);
   ...
   */
  testModelMemory(
    model: { name: string; properties: ReadonlyMap<string, any> }
  ): MemoryMetrics {
    ...
    console.log(`   Memory per property: ${metrics.memoryPerPropertyMB.toFixed(4)}MB`);
    console.log(`   Acceptable usage: ${metrics.acceptableMemoryUsage ? '✅' : '❌'}`);
...
describe("Memory Validation Tests", () => {
  ...
  it("should validate memory efficiency across model sizes", () => {
    ...
    expect(results.summary.keyMetrics.averageMemoryPerPropertyMB).toBeLessThan(0.1);
    
    console.log(`Memory validation completed with status: ${results.summary.status}`);
  });

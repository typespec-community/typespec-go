...
describe("Native TypeSpec uint Support", () => {
  it("should handle native uint32, uint8, uint16, uint64 types directly", () => {
    ...
    const userModel = {
      ...
      properties: new Map([
        ...
        ["balance", { name: "balance", type: { kind: "Int64" }, optional: true }], // Signed when negative possible
      ]),
    };
  ...
  it("should demonstrate performance of native type mapping", () => {
    ...
    console.log(`   ${totalMappings} native type mappings in ${totalTime.toFixed(2)}ms`);

 ...
 */
export const DEFAULT_EMITTER_OPTIONS: Required<Omit<GoEmitterOptions, keyof typeof NEVER_CONFIGURED>> = {
  outputDir: "./generated",
...
export class EmitterConfigValidator {
  ...
  static validateOptions(options?: GoEmitterOptions): GoEmitterOptions {
    ...
    if (options.outputDir && typeof options.outputDir !== "string") {
      throw new Error(`outputDir must be a string, got: ${typeof options.outputDir}`);
    }

    // Validate naming pattern
    if (options.namingPattern && !["snake_case", "PascalCase"].includes(options.namingPattern)) {
      throw new Error(`namingPattern must be 'snake_case' or 'PascalCase', got: ${options.namingPattern}`);
    }

    // Validate json tag style
    if (options.jsonTagStyle && !["snake_case", "camelCase"].includes(options.jsonTagStyle)) {
      throw new Error(`jsonTagStyle must be 'snake_case' or 'camelCase', got: ${options.jsonTagStyle}`);
    }

    // Validate pointer policy
    if (options.pointerPolicy && !["all", "optional_only", "primitives_only"].includes(options.pointerPolicy)) {
      throw new Error(`pointerPolicy must be 'all', 'optional_only', or 'primitives_only', got: ${options.pointerPolicy}`);
    }

    // Validate uint policy
    if (options.uintPolicy && !["auto", "int_only", "prefer_uint"].includes(options.uintPolicy)) {
      throw new Error(`uintPolicy must be 'auto', 'int_only', or 'prefer_uint', got: ${options.uintPolicy}`);
    }

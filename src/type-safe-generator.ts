...
export class TypeSafeGoGenerator {
   ...
   */
  generateModel(model: TypeSpecPropertyNode & { name: string }): string {
  ...
  generateStruct(name: string, properties: TypeSpecPropertyNode[]): string {
    ...
    const fields = properties.map(prop => 
      OptionalFieldPolicy.generateField(prop)
   ...
   */
  private extractProperties(model: any): TypeSpecPropertyNode[] {
    ...
    const properties: TypeSpecPropertyNode[] = [];
    
    for (const [propName, prop] of model.properties) {
      properties.push({
        name: propName,
        type: prop.type as TypeSpecTypeNode,
   ...
   */
  private formatField(field: GoField): string {

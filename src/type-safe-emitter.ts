...
import { TypeSpecTypeNode, TypeSpecPropertyNode } from "./types/type-spec-types.js";
 ...
 */
function extractModelsFromProgram(program: Program): TypeSpecPropertyNode[] {
  const models: TypeSpecPropertyNode[] = [];
  
  // Use working patterns from baseline - check if models exist
  // Note: Program API may vary, use any available method
  if ((program as any).models && (program as any).models.size > 0) {
    for (const [namespace, namespaceModels] of (program as any).models) {
      for (const model of namespaceModels) {
        // Type-safe model extraction
        const typedModel: TypeSpecPropertyNode = {
          name: model.name,
          type: {
            kind: "Model",
            name: model.name,
            properties: model.properties as ReadonlyMap<string, TypeSpecPropertyNode>
          } as TypeSpecTypeNode,
  ...
  if (models.length === 0) {
    ...
    if ((program as any).namespaces) {
      for (const namespace of (program as any).namespaces.values()) {
        if (namespace.models) {
 ...
 */
function validateGeneratedGoCode(goFiles: Record<string, string>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];

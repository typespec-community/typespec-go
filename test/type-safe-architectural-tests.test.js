...
import { TypeSafeMapper, TYPE_SPEC_MAPPINGS } from "../src/mappers/type-safe-mapper.js";
...
describe("TypeSpec Go Emitter - Type-Safe Architecture", () => {
    ...
    describe("BDD: Type Safety - Zero 'any' Types", () => {
        it("GIVEN: TypeSpec type nodes - WHEN: Mapping to Go - THEN: Zero 'any' types", () => {
            ...
            const stringNode = { kind: "String" };
            const uint32Node = { kind: "Uint32" };
    ...
    describe("BDD: Exhaustive Type Matching - Compile-Time Safety", () => {
        it("GIVEN: All TypeSpec types - WHEN: Accessing mappings - THEN: Compile-time exhaustive matching", () => {
            ...
            expectedKinds.forEach(kind => {
                strictEqual(typeSpecKinds.includes(kind), true, `Should have mapping for ${kind} type`);
            });
            // Critical: Complete type coverage
            const missingKinds = expectedKinds.filter(kind => !typeSpecKinds.includes(kind));
            strictEqual(missingKinds.length, 0, `Should have no missing kinds. Missing: ${missingKinds.join(', ')}`);
        });
    });
    describe("BDD: Complete Uint Support - Professional Go Coverage", () => {
        it("GIVEN: Uint TypeSpec types - WHEN: Mapping to Go - THEN: All uint types supported", () => {
            ...
            uintTypes.forEach(uintKind => {
                ...
                strictEqual(mapping.goType, expectedGoType, `${uintKind} should map to ${expectedGoType}`);
                // Then: Should use pointer for optional
                strictEqual(mapping.usePointerForOptional, true, `${uintKind} should use pointer for optional`);
            });
        });
    });
    ...
    describe("Architecture: Domain Separation", () => {
        it("GIVEN: Domain components - WHEN: Analyzing dependencies - THEN: Clean separation", () => {
            ...
            strictEqual(typeMapperIsSelfContained, true, "Type mapping domain should be self-contained");
            ...
            strictEqual(optionalPolicyIsIndependent, true, "Optional field policy should be independent");
            ...
            strictEqual(generatorComposesDomains, true, "Generator should properly compose domains");
        });
    });
    describe("Architecture: Type Safety Enforcement", () => {
        it("GIVEN: Type-safe components - WHEN: Checking types - THEN: No 'any' usage", () => {
            ...
            strictEqual(typeof stringMapping.goType, "string", "Should have typed goType");
            strictEqual(typeof stringMapping.usePointerForOptional, "boolean", "Should have typed usePointerForOptional");
            // Critical: No 'any' in returned type
            const hasAnyInMapping = JSON.stringify(stringMapping).includes('any');
            strictEqual(hasAnyInMapping, false, "Mapping should contain no 'any' types");
        });
    });
    // =======================
    // PERFORMANCE TESTS
